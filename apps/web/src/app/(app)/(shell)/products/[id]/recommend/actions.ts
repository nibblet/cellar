...
export async function submitRecommend(_prev: State, formData: FormData): Promise<State> {
  ...
  const releaseLabelSourceRaw = (formData.get("release_label_source") as string | null)?.trim();
  const releaseLabelSource =
    releaseLabelSourceRaw === "vision" || releaseLabelSourceRaw === "member"
      ? releaseLabelSourceRaw
      : null;
