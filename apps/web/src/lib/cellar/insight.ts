...
export async function loadShelfProducts(
  ...
  const { data } = await supabase
    ...
    .eq("have", true)
    // Mirror the member-facing catalog cut-back so Winston narrates the same
    // set the member sees — excluded twins / de-listed bottles must not inflate
    // counts. Cigars default catalog_included=true, so this only trims bourbons.
